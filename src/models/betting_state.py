...
class BettingState:
    ...
    def __init__(self, pos: int, board_state: str, board_cards: List[Card], game_round: GameRound, 
        players: List[str], player_actions: Dict[str, List[Tuple[str, int]]] = None, previous_state: 'BettingState' = None):
    ...
    def is_pre_flop_state(self):
        return True if self._board_state == 'pre-flop' else False

    def is_post_flop_state(self):
        return True if self._board_state == 'post-flop' else False

    def is_turn_state(self):
        return True if self._board_state == 'turn' else False

    def is_river_state(self):
        return True if self._board_state == 'river' else False
    ...
    def is_round_complete(self):
        ...
        if (all_players_called_action):
            ...
            number_of_players_folded = len([player for (player, actions) 
                in self._player_actions.items() if (actions[-1][0] == 'fold')])
    ...
    def get_valid_actions(self, player: str, player_pot: int, small_blind_stake: int) -> List[Tuple[str, int, int]]:
        ...
        biggest_betting_chip = (max([sum([x[1] for x in actions]) 
            for (pl, actions) in self._player_actions.items() if actions[-1][0] != 'fold' and pl != player])
    ...
    def get_round_total_pot(self):
        ...
        return previous_state_total_pot + sum([betting_chip for actions in self._player_actions.values() for (action, betting_chip) in actions])
